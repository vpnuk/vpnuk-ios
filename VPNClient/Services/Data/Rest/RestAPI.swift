...
//                        ]
    ...
    func getServersVersion(callback: @escaping (_ servers: Result<ServersVersionDTO>) -> ()) {
        request(URL(string: "https://www.vpnuk.info/serverlist/versions.xml")!, method: .get, parameters: nil, encoding: URLEncoding.default, headers: nil)
            .responseData(queue: DispatchQueue.global(qos: .userInitiated)) { (response) in
                if let error = response.error {
                    DispatchQueue.main.async {
                        callback(.failure(error))
